...
csvFolder = r"F:\航图\中国民航国内航空资料汇编 NAIP 2402\csv"
# 获取文件
file_list = []
walking(inputFolder)
...
# 获取可用文件列表
chartName, runways = get_info(csvFolder, "航路")
# 不能自动读取的文件
fatal_list = []
# 断点续读
crash_airport = None
...
for iPath in file_list:
    ...
    try:
        ...
        printf("{}发生错误".format(chartNow), 2)
        fatal_list.append(chartNow[:4])
    pdfNow.close()
...
fatal_list.sort()
# 这里10 ['ZBCZ', 'ZGNN', 'ZLJC', 'ZLXY', 'ZLZY', 'ZSGZ', 'ZUJZ', 'ZULS', 'ZWBL', 'ZYDD']
# 检查1  ['ZLHB']
# 问题文件不一致 注意核对
